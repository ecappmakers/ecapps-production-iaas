...
if __name__ == '__main__':
    ...
    for db_dir in sorted(os.listdir(schemas_dir)):
        ...
        # Get password from environment variable using secret_name with mapping
        db_user_pass = get_secret_value(secret_name)
        
        if not db_user_pass:
            print(f"      ❌ Secret '{secret_name}' not found in environment")
            # Show available secrets for debugging
            db_secrets = [k for k in os.environ.keys() if k.startswith('DB_')]
            print(f"      📝 Available DB secrets: {', '.join(db_secrets) if db_secrets else 'none'}")
            missing_secrets.append(secret_name)
            users_skipped += 1
            continue
